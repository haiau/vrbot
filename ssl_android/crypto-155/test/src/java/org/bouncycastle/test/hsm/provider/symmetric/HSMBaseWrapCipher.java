...
public abstract class HSMBaseWrapCipher
    extends CipherSpi
    implements PBE
{
    //
    ...
    protected byte[] getHSMEncoded(Key key) {
        if (key instanceof HSMSecretKeySpec) {
            return ((HSMSecretKeySpec) key).hsmGetEncoded();
        } else {
    ...
    protected int engineGetKeySize(
        Key     key)
    {
        return getHSMEncoded(key).length;
    }
    ...
    {
        byte[] encoded = getHSMEncoded(key);
        if (encoded == null)
        {
